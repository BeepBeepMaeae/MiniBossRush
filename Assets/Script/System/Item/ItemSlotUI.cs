...
{
    public enum SlotKind { HealthPotion, StaminaPotion }
    ...
    public void UpdateCount(int count) => SetCount(count);
    ...
    {
        AudioClip clip = sfxUse;
        if (clip == null)
            clip = (slotKind == SlotKind.HealthPotion) ? sfxUseHealthDefault : sfxUseStaminaDefault;
        if (clip == null) return;
